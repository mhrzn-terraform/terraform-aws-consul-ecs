locals {
  ...
  internal_load_balancer = var.internal_lb_enabled ? [{
    target_group_arn = aws_lb_target_group.this_internal[0].arn
    container_name   = "consul-server"
    container_port   = 8500
  }] : []
...
resource "aws_lb_target_group" "this_internal" {
...
resource "aws_lb_listener" "this_internal" {
  count             = var.internal_lb_enabled ? 1 : 0
  load_balancer_arn = aws_lb.this_internal[count.index].arn
  port              = "8500"
  protocol          = "HTTP"
  default_action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.this_internal[count.index].arn
  }
}