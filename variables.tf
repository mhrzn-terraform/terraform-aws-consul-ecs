...
variable "internal_lb_enabled" {
  description = "Whether to create an internal ALB for the server task. Useful for accessing the UI."
  type        = bool
  default     = false
}

variable "internal_lb_subnets" {
  description = "Subnet IDs to attach to the load balancer. NOTE: These must be private subnets if you wish to access the load balancer internally."
  type        = list(string)
  default     = null
}